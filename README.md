...
For this guide, we have chosen to host the example file on Dropbox, where you can access it at this link: <DROPBOX_LINK>dl=0. To continue the guide, when you go to import data, you will have to write the dropbox link into the report, and change the last number from `0` to `1`. 

For example, you will change `<DROPBOX_LINK>dl=0` to `<DROPBOX_LINK>?dl=1`. The changes allows neo4j to download the data and import it into AuraDB. This link will be kept available for you to try and use. 
